...
void PrototypeGEvaluator::notify_initial_state(const GlobalState &initial_state) {
    auto &cache_entry = heuristic_cache[initial_state];
    cache_entry.h = 0;
    cache_entry.dirty = false;
}

void PrototypeGEvaluator::notify_state_transition(const GlobalState &parent_state,
                                         OperatorID op_id,
